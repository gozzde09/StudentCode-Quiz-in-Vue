...
onMounted(async () => {
  try {
    ...
    for (const quizItem of quizData.value) {
      const correctAnswers = quizItem.correct_answers;

      for (const key in correctAnswers) {
        if (correctAnswers[key] === "true") {
          correctAnswersArray.push(key);
          console.log(key);
        }
      }
    }
...
const nextQuestion = () => {
  ...
  if (selectedAnswers.value !== currentQuestion.value.correct_answer) {
    // Show the correct answer when a wrong answer is selected
    selectedAnswers.value = 'red';
    selectedAnswers.value = currentQuestion.value.correct_answer;
    // Uncomment the following line to display the correct answer
    console.log(`Correct answer: ${currentQuestion.value.correct_answer}`);
  }
  setTimeout(() => {
...
<template>
  ...
  <div v-if="quizData" class="container d-flex flex-column">
    ...
    <h2 class="mx-auto my-3" style="max-width:60%"> {{ currentQuestionIndex+1 }}. {{ currentQuestion.question }}</h2>
