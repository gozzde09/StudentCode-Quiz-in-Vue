<script setup>
  ...
  const QuizStart = async () => {
    ...
    router.push(
      `QuizPage/${category.value}/${difficulty.value}/${questionAmount.value}`
    )
    try {
      await axios.get(
        `https://quizapi.io/api/v1/questions?apiKey=Fn3mWDcTNToCVxnnLtiH2OXe9XSGTcpUFpl3SUUq&limit=${questionAmount.value}&tags=${category.value}&difficulty=${difficulty.value}`
      )
    } catch (error) {
    ...
    <div class="container justify-content-center my-3">
      ...
      <div class="btn-group" role="group" aria-label="Topic">
        ...
        <template v-for="topic in topics" :key="topic.id">
          <input
            v-model="category"
            @click="categoryStore.setCategory(topic.id)"
            type="radio"
            class="btn-check"
            :name="'topic'"
            :id="topic.id"
